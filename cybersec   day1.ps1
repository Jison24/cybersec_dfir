...
client-identifier abcd.1234.cafb
...
ephone-dn 10
 number ~~98
Ephone 1
  Mac-address cafe.face.baba
  type 8945
  button 1:8 2:7 3:6 4:5
  restart
Ephone 2
  Mac-address fafa.caca.baba
  type 8945
...
config t
   ...
   Int Gi 0/0/0
     ...
     ip add 10.~~.~~.1 255.255.255.0
	 no shutdown
   Int Gi 0/0/1
     description PLDT-ME-WAN
	 ip add 200.0.0.~~ 255.255.255.0
	 no shutdown
   Int Loopback 0
     description VIRTUALIP-FOR-ROUTING
	 ip add ~~.0.0.1 255.255.255.255
 end
...
network 10.~~.~~.0  0.0.0.255 area 0
network ~~.0.0.1  0.0.0.0 area 0
Interface gi 0/0/0
...
conf t
 ...
 NO ip route 10.~~.0.0 255.255.0.0 200.0.0.~~
 ...
 ip route 10.~~.0.0 255.255.0.0 10.~~.~~.4
 NO ip route 10.~~.0.0 255.255.0.0 200.0.0.~~
 end
...
cmd
route  add   10.0.0.0   mask   255.0.0.0    10.~~.1.4
route  add  200.0.0.0   mask  255.255.255.0   10.~~.1.4
...
http://docwiki.cisco.com/wiki/Cisco~~IOS~~Voice~~Troubleshooting~~and~~Monitoring~~--~~Tcl~~IVR~~Troubleshooting
...
!
 voice register global
  mode cme
  source-address 10._.100.8 port 5060
  max-dn 12
  max-pool 12
